...
namespace ParkingServiceServer.Menager
{
    public class ServiceCertValidator : X509CertificateValidator
    {
        ...
        public override void Validate(X509Certificate2 certificate)
        {
            ...
            {
                SecurityHelper.AuditAuthenticationFailure("Certificate is not from the valid issuer.");
                throw new Exception("Certificate is not from the valid issuer.");
            }

            if(Formatter.ParseName(WindowsIdentity.GetCurrent().Name) == "primary")
            {
                SecurityHelper.AuditAuthenticationSuccess("secondary");
            }
            else
            {
                SecurityHelper.AuditAuthenticationSuccess("primary");
            }
